...
$che_query=mysql_query("SELECT id
FROM  `scie_k2_categories` where parent=2");

$c_sub=array();
while($che_subcategories=mysql_fetch_array($che_query))
{
array_push($c_sub, $che_subcategories['id']);
}
$che_subStr = implode('","', $c_sub);
$che_subStr_fnl='"'.$che_subStr.'"';

$che_scatids = mysql_query("SELECT * FROM scie_k2_categories WHERE parent in ($che_subStr_fnl) AND published = '1' order by id DESC LIMIT 6") or die(mysql_error());
/*-------------- CHEMISTRY NEW TOPICS END ------------------*/
?>
<h3 style="font-family:'NewCicleFina';margin:0px 0px 10px 0px;padding:0px;">Chemistry</h3>
<div class="container">
    <div class="row">
        ...
        <div class="col-sm-9 col-md-9">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                	<?php
while($catnames_chemistry=mysql_fetch_array($che_scatids))
{
...
?>
   <ul class="thumbnails" id="hover-cap" style="margin:0px;padding:0px;">
                        ...
                        <li style="width:190px;float:left;list-style:none;margin-right:15px;">
                            <?php
                            $checatid = $catnames_chemistry['id'];
                            //Fetch all items for the catid
							
                            $c_k2items = mysql_query("SELECT * FROM scie_k2_items WHERE catid = '$checatid' ORDER BY ordering ASC") or die(mysql_error());
                            $i = 0;
                            // Matching using string of name field from k2 categories table to find its id in menu table
                            $ctopicname = $catnames_chemistry['name'];
							
                            $c_menuitem = mysql_query("SELECT id FROM scie_menu WHERE level = '3' AND published = '1' AND title LIKE '$ctopicname%'") or die(mysql_error());
                            $c_menuitemid_show = mysql_fetch_array($c_menuitem);
                            $c_menuitemid = $c_menuitemid_show['id'];
                            ?>
                            <div class="thumbnail">
                                <div class="caption">
								     
                                    <h4 style="margin-top:0px;text-align:center;">
                                        <?php echo $catnames_chemistry['name']; ?>
                                    </h4>
				  <?php   if (in_array('2', $groups) && !in_array('10',$groups))  {  ?>
									
									 		    <p>
                                        <?php while($c_k2items_show = mysql_fetch_array($c_k2items)) {
                                        $i = $i + 1;
										if($c_k2items_show['access']==2)  {
                                        ?>
                                        <!-- Looping to show Video, Concept and Assessment-->
										
                                        <?php if ($i == 1) { ?>
                                        <a href="index.php?option=com_k2&view=item&layout=item&id=<?php echo $c_k2items_show['id'];?>&Itemid=<?php echo $c_menuitemid;?>&catid=<?php echo $c_k2items_show['catid'];?>" class="btn btn-default btn-xs" style="margin:0px 5px 10px 12px;"><span class="glyphicon glyphicon-play" aria-hidden="true"></span>&nbsp;Video</a>
                                        <?php }
                                        if ($i == 2) { ?>
                                        <a href="index.php?option=com_k2&view=item&layout=item&id=<?php echo $c_k2items_show['id'];?>&Itemid=<?php echo $c_menuitemid;?>&catid=<?php echo $c_k2items_show['catid'];?>" class="btn btn-default btn-xs" style="margin:0px 0px 10px 7px;"><span class="glyphicon glyphicon-list" aria-hidden="true"></span>&nbsp;Concept</a>
                                        <?php } 
                                        if ($i == 3) { ?>
                                        <br/>
                                        <a href="index.php?option=com_k2&view=item&layout=item&id=<?php echo $c_k2items_show['id'];?>&Itemid=<?php echo $c_menuitemid;?>&catid=<?php echo $c_k2items_show['catid'];?>" class="btn btn-default btn-xs" style="margin:0px 0px 0px 40px;"><span class="glyphicon glyphicon-file" aria-hidden="true"></span>&nbsp;Assessment</a>
                                ...
                                <img style="width:100%;" src="media/k2/categories/<?php echo $catnames_chemistry['image'];?>">
                            </div>
                        </li>
                    </ul>
<?php
        ...
        <div class="col-sm-3 col-md-3">
            <div class="panel panel-default">
                ...
                <div class="list-group">
                    ...
                    $p_tracks = mysql_query("SELECT * FROM scie_k2_categories WHERE parent = '2'") or die(mysql_error());
                    while($p_tracks_show = mysql_fetch_array($p_tracks)) { 
                        ...
                        <a href="index.php?option=com_content&abcon3diy5usumgbagfdsdlu=<?php echo $id; ?>&view=article&id=13&Itemid=143&Itemid=<?php echo $p_track_menu_id_show[id];?>" class="list-group-item"><span class="glyphicon glyphicon-folder-close"></span>&nbsp;&nbsp;&nbsp;<?php echo $p_tracks_show['name'];?><span class="badge"><?php echo $count;?></span></a>
                    <?php }} ?>
                </div>
            </div>
